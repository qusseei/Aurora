...
void User::set()
{
    ...
    if(j == 123)
    {
        ...
        ofstream ofile("管理员账号.txt", ios_base::app);
...
void User::load()
{
    ...
    ifstream ifile;
    ifile.open("管理员账号.txt", ios_base::in);
    while(ifile.good())
    {
...
void manager_use::setstu()
{
    ...
    for(;;)
    {
        ...
        cout << endl << "\t\t\t请输入姓名：";
        cin >> members[i].name;
        ofstream ofile("学生信息.txt", ios_base::app);
    ...
    for(int i = 0; i < 20; i++)
    {
        ...
        if(num == information[i][0])
        {
            ...
            if(members[n].money - i > 0)
            {
                ...
                ofstream g("学生信息.txt", ios::binary);
                for(i = 0; i < sum; i++)
                {
...
{
    FILE *fp;
    if((fp = fopen("评价.txt", "a")) == NULL)
    {
        printf("can't open the file 评价.txt\n");
        exit(0);
    }
    fclose(fp);
    //将菜谱中的菜品全部读出来存在FoodMssage数组中
    if((fp = fopen("菜谱.txt", "r")) == NULL)
    {
        printf("can't open the file 菜谱.txt\n");
        exit(0);
    }
    int num = 0;
    char foodname[20];
    ...
    fclose(fp);
    //将评价.txt中的评价信息全部存入到FoodMssage数组中
    if((fp = fopen("评价.txt", "r")) == NULL)
    {
        printf("can't open the file 评价.txt\n");
        exit(0);
    }
    ...
    fclose(fp);
    //将FoodMssage数组中更新后的评价信息全部存入到评价.txt中
    if((fp = fopen("评价.txt", "w")) == NULL)
    {
        printf("can't open the file 评价.txt\n");
        exit(0);
    }
